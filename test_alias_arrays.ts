...
const program = ts.createProgram(['alias-array-test.ts'], {
  target: ts.ScriptTarget.Latest,
  module: ts.ModuleKind.ESNext,
}, {
  getSourceFile: (fileName) => fileName === 'alias-array-test.ts' ? sourceFile : undefined,
  writeFile: () => {},
  getCurrentDirectory: () => '',
  getDirectories: () => [],
  fileExists: () => true,
...
for (const propName of testCases) {
  ...
  if (ts.isTypeReferenceNode(property.type) && ts.isIdentifier(property.type.typeName)) {
    ...
    if (symbol?.declarations) {
      ...
      for (const decl of symbol.declarations) {
        if (ts.isTypeAliasDeclaration(decl)) {
          console.log(`Type alias declaration type kind: ${ts.SyntaxKind[decl.type?.kind || 0]}`);
          console.log(`Type alias declaration text: ${decl.type?.getText(sourceFile) || 'none'}`);
          if (decl.type) {
            if (ts.isArrayTypeNode(decl.type)) {
              ...
              if (ts.isIdentifier(decl.type.typeName) && decl.type.typeName.text === 'Array') {
